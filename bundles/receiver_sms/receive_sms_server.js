...
console.log("The receive_sms_server script will be blocked two seconds to be sure the RabbitMQ server is running.");
        ...
        AMQP.connect(SYS.CONSTANTS.CONNECT_TO, (err, connection) => {
            ...
            connection.createChannel((err, channel) => {
                ...
                channel.assertQueue(SYS.CONSTANTS.SMS_QUEUE_NAME, { exclusive: true }, (err, q) => {
                    ...
                    // Clean exit
                    if(err) { SYS.H.errHandler(`while trying to assert the ${SYS.CONSTANTS.SMS_QUEUE_NAME} queue`, err); }
